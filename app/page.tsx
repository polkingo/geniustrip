...
import { Loader2, Calendar, Euro, ChevronRight, Plus, Trash2, MapPin, Settings2, ChevronDown, Menu, BarChart2, LineChart, Rocket, X, ArrowUpRight, Compass } from "lucide-react";
import { PieChart, Pie, Tooltip as RTooltip, ResponsiveContainer, Legend, Cell } from "recharts";

// ---------- Utils ----------
const currency = (n, cur = "EUR") =>
...
function pickBestDates(earliestDeparture, latestReturn, days) {
  ...
  let best = { depart: earliestDeparture, return: latestReturn, score: Infinity };
...
function estimateTrip(
  ...
  // --- New: estimate nightly prices per city and sort destinations by cheapest ---
  const estimateNightly = (cityName) => {
    const base = budget < 400 ? rand(25, 60) : rand(60, 140);
    // If hostels allowed, assume ~15% cheaper options are possible
  ...
  // Sort ascending nightly price => cheaper cities earlier in route
  cityInfos.sort((a, b) => a.nightly - b.nightly);
  const sortedTos = cityInfos.map((c) => c.city);

  // --- New: allocate nights biased toward cheaper cities ---
  // weights = 1 / nightly, normalized so min 1 night each and sum = days
  const weights = cityInfos.map((c) => 1 / Math.max(1, c.nightly));
  const sumW = weights.reduce((s, w) => s + w, 0);
  // ensure at least 1 night per city
  let nights = cityInfos.map(() => 1);
  let remaining = Math.max(days - nights.length, 0);
  // provisional fractional allocation for remaining nights
  ...
  // Build flight legs using sorted order
  const legs = [];
  ...
  // Build stays using the biased nights + the nightly estimates we computed
  const stays = cityInfos.map((info, i) => {
    const city = info.city;
    const pricePerNight = info.nightly;
    const total = pricePerNight * nights[i];
    const cityName = city.split(" (")[0];
    const hotels = allowHostels
      ? [
          {
            name: `${cityName} City Hostel`,
            pricePerNight: Math.max(18, Math.round(pricePerNight * 0.9)),
          },
          { name: `${cityName} Central Inn`, pricePerNight },
        ]
      : [
          { name: `${cityName} Boutique Hotel`, pricePerNight: pricePerNight + 25 },
          { name: `${cityName} Central Inn`, pricePerNight },
        ];
...
function __runTests() {
  // Test 1: Date picker picks inside window
  ...
  // Test 2: estimateTrip should return legs = destinations + 1 (return leg)
  ...
  // Test 3: route should be sorted by nightly cost ascending
  const nightlyByCity = Object.fromEntries(e.stays.map(s => [s.city, s.pricePerNight]));
  const isNonDecreasing = e.route.every((c, i, arr) => i === 0 || nightlyByCity[arr[i-1]] <= nightlyByCity[c]);
  console.assert(isNonDecreasing, "route not sorted by cheapest nightly rate");
  // Test 4: nights sum equals days and at least 1 per city
  const nightsSum = e.stays.reduce((sum, s) => sum + s.nights, 0);
  const allAtLeastOne = e.stays.every(s => s.nights >= 1);
  console.assert(nightsSum === 7 && allAtLeastOne, `night allocation invalid: sum=${nightsSum}, all>=1=${allAtLeastOne}`);
}

// ---------- Page Component ----------
export default function GeniusTripApp() {
  // Header state (was missing before)
  const [menuOpen, setMenuOpen] = useState(false);

  // Planner state
  const [from, setFrom] = useState("Lisbon (LIS)");
  const [toInput, setToInput] = useState("");
  const [tos, setTos] = useState(["Paris (CDG)"]); // multiple destinations
  ...
  // Small runtime sanity tests (helpful in dev, no visual effect)
  useEffect(() => {
  ...
  const handleRemoveTo = (t) => setTos((prev) => prev.filter((x) => x !== t));
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-[linear-gradient(180deg,rgba(219,234,254,.7),#fff_20%)]">
              ...
              {!loading && result && (
                <div className="space-y-6">
                  <div className="p-4 rounded-xl border bg-blue-50/60 border-blue-100">
                    ...
                    <div className="text-xs text-slate-600 mt-1">Best dates: <strong>{result.chosen.depart || "?"}</strong> → <strong>{result.chosen.return || "?"}</strong> · Days: <strong>{result.days}</strong> · Route optimized for price: <strong>{result.route?.join(" → ")}</strong> · Stops pref: <strong>{result.prefs.stopovers===0? 'non‑stop' : result.prefs.stopovers===3? '3+': result.prefs.stopovers}</strong> · Hostels: <strong>{result.prefs.allowHostels? 'allowed':'no hostels'}</strong>.</div>
                  ...
                  <div className="space-y-2">
                    ...
                    {result.legs.map((leg, i) => (
                      <div
                        key={i}
                        className="flex justify-between bg-white border rounded-xl px-3 py-2 text-sm"
                      >
                        <span>
                          {leg.from} → {leg.to}{" "}
                    ...
                    {result.stays.map((s, i) => (
                      <div key={i} className="bg-white border rounded-xl p-3 text-sm">
                        ...
                        <div className="text-xs text-slate-500 mb-2">
                          {s.nights} night{s.nights > 1 ? "s" : ""} × {currency(s.pricePerNight)}
                          /night
                        </div>
                      ...
                      <div key={i} className="bg-white border rounded-xl p-3">
                        ...
                        <div className="grid md:grid-cols-2 gap-2 text-sm text-slate-700">
                          {c.items.map((it, j) => (
                            <div
                              key={j}
                              className="flex items-center justify-between bg-slate-50 border rounded-xl px-3 py-2"
                            >
                              <span>
          ...
          <div className="max-w-6xl mx-auto px-4 py-12">
            ...
            <div className="mt-6 flex flex-wrap gap-2 text-xs">
              {[
                ...
                <span
                  key={t}
                  className="px-3 py-1 rounded-full bg-slate-100 text-slate-600 border"
                >
                  {t}
                </span>
              ))}
        ...
        <section
          id="reviews"
          className="bg-[linear-gradient(180deg,rgba(219,234,254,.35),#fff_40%)] border-y"
        >
          <div className="max-w-6xl mx-auto px-4 py-12">
            ...
            <p className="text-slate-600 mt-1">
              We’re here to help you make the right decision. Explore what
              travellers say about GeniusTrip.ai.
            </p>
              ...
              {[
                {
                  name: "Brendan",
                  role: "owner of Goog",
                  text: "A fantastic tool! It saved us time and money beyond expectations.",
                  avatar: "B",
                },
                {
                  name: "jacychan",
                  role: "owner of Breit",
                  text: "Simply exceptional! Fast, efficient, and packed with the essentials.",
                  avatar: "J",
                },
                {
                  name: "gaat",
                ...
                },
                {
                  name: "Wilson",
                  role: "owner of Talik",
                  text: "An absolute must‑have! Intuitive features and real‑time insights.",
                  avatar: "W",
                },
                {
                  name: "jamesli",
                  role: "owner of Candto",
                  text: "An amazing platform! It streamlined our workflow effortlessly.",
                  avatar: "J",
                },
                {
                  name: "anna",
                  role: "owner of Hanko",
                  text: "A true game‑changer! Powerful features and seamless UX.",
                  avatar: "A",
                },
                {
                  name: "maya",
                  role: "owner of Janio",
                  text: "Incredible tool! Saved time and resources while improving efficiency.",
                  avatar: "M",
                },
                {
                  name: "shallot",
                  role: "owner of Tanko",
                  text: "A game changer! Optimized our process and delivered great value.",
                  avatar: "S",
                },
                {
                  name: "sofia",
                  role: "frequent flyer",
                  text: "City‑aware suggestions were on point and many were free!",
                  avatar: "S",
                },
              ].map((r, idx) => (
                <div key={idx} className="rounded-2xl border bg-white p-5 shadow-sm relative">
                  ...
                  <p className="text-slate-800 italic">"{r.text}"</p>
              ...
              {[
                {
                  ...
                  a: "We’ll use API responses directly from partners (e.g., Kiwi/Booking) and validate ranges before showing them.",
                },
                {
                  q: "Can I integrate this with other tools?",
                  a: "Yes — webhooks and export endpoints are planned so you can send itineraries to calendars or CRMs.",
                },
                {
                  q: "How long does it take to get started?",
                  a: "You can generate a plan in under 20 seconds. Live booking links will arrive once partner APIs are enabled.",
                },
                {
                  q: "What kind of support do you offer?",
                  a: "Priority email/chat for paid users; SLA for partners.",
                },
                {
                  q: "Is my data secure?",
                  a: "We’ll store only what’s needed for your plan and delete on request. HTTPS everywhere.",
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className={`border-b last:border-b-0 ${
                    faqOpen === i ? "bg-slate-50/60" : ""
                  }`}
                >
                  <button
